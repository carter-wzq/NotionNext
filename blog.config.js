...
const BLOG = {
  // Vercel/Node 访问 www.notion.so/api/v3 常会 fetch failed（530/无响应）。
  // 若环境变量仍指向 www.notion.so，忽略并用已发布的 notion.site。
  // 文档：https://docs.tangly1024.com/article/notion-next-api_base_url
  API_BASE_URL: (() => {
    const envUrl = process.env.API_BASE_URL
    if (envUrl && !/www\.notion\.so/i.test(envUrl)) return envUrl
    return 'https://eminent-fountain-944.notion.site/api/v3'
  })(),
  // Important page_id！！！Duplicate Template from  https://tanghh.notion.site/02ab3b8678004aa69e9e415905ef32a5
  NOTION_PAGE_ID:
    process.env.NOTION_PAGE_ID ||
    '94a9b5e02328839cab9101f71f0ee990',
  ...
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 600, // 更新缓存间隔 单位(秒)；即每个页面有600秒(10分钟)的纯静态期、此期间无论多少次访问都不会抓取notion数据；调大该值有助于节省Vercel资源、同时提升访问速率，但也会使文章更新有延迟。
  ...
  AUTHOR: process.env.NEXT_PUBLIC_AUTHOR || 'SignalMelo', // 您的昵称 例如 tangly1024
  BIO: process.env.NEXT_PUBLIC_BIO || 'Turn social signals into clear growth decisions', // 作者简介
  LINK: process.env.NEXT_PUBLIC_LINK || 'https://www.signalmelo.com', // 网站地址
  KEYWORDS: process.env.NEXT_PUBLIC_KEYWORD || 'social listening, social media monitoring, reddit marketing', // 网站关键词 英文逗号隔开
  ...
  GREETING_WORDS:
    ...
    'Turn social signals into clear growth decisions',
