...
import RedisCache, { redisClient } from './redis_cache'
...
/** 仅 next dev：设置 NOTION_DEV_CACHE_SECONDS 后关闭本地 file 缓存，配合 memory 短 TTL，改 Notion 后无需手删 .next/cache/notion */
function isDevNotionShortCacheEnabled() {
...
const enableLocalCache =
  isBuildPhase || (!BLOG.isProd && !isDevNotionShortCacheEnabled())
...
/** 识别 Notion 拉取失败时的 EmptyData 兜底（含 Redis 里旧缓存、无 __notionEmpty 标记的版本） */
function isNotionEmptyFallback(data) {
  if (!data || typeof data !== 'object') return false
  if (data.__notionEmpty) return true
  ...
  return (
    page?.slug === 'oops' ||
    title.includes('无法获取Notion数据') ||
    title.includes('Notion_ID')
  )
}

function isUsableCacheValue(data) {
  if (data == null) return false
  if (isNotionEmptyFallback(data)) return false
  if (Array.isArray(data)) return data.length > 0
  return true
}

function isCacheableValue(data) {
  if (data == null) return false
  if (isNotionEmptyFallback(data)) return false
  return true
}
  ...
  if (isBuildPhase) {
    const promise = withFileLock(
      ...
      async () => {
        ...
        } else if (data?.__notionEmpty) {
          cacheLog('SKIP-SET', key, 'empty notion fallback not cached')
        }
...
/** 删除 Redis 中某前缀的全部 key（用于刷掉过期的 page_block_* / site_*） */
export async function delCacheByPrefix(prefix) {
  if (!hasRedis || typeof redisClient?.scanStream !== 'function') {
    await delCacheData(prefix)
    return { deleted: 0, prefix }
  }
  const keys = []
  ...
  return { deleted: unique.length, prefix }
}
