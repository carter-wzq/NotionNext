...
const FALLBACK_API_BASE_URL = 'https://eminent-fountain-944.notion.site/api/v3'

function resolveApiBaseUrl() {
  const configured = BLOG.API_BASE_URL || FALLBACK_API_BASE_URL
  if (/www\.notion\.so/i.test(configured)) return FALLBACK_API_BASE_URL
  return configured
}
...
function getRawNotion() {
  if (!globalStore.notion) {
    ...
    console.log('[NotionAPI] using apiBaseUrl:', apiBaseUrl)
    globalStore.notion = createNotionClient(apiBaseUrl)
  }
