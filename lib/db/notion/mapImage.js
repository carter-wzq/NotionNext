...
/**
 * file.notion.com 带短期签名，过期后返回 419 Resource Expired。
 * 还原为 attachment:{fileId}:{filename}，再走 notion.so/image 代理即可长期有效。
 */
const rewriteFileNotionUrl = url => {
  if (!url || typeof url !== 'string') return url
  if (!url.includes('file.notion.com') && !url.includes('file.notion.so')) {
    return url
  }
  try {
    ...
    // /f/f/{spaceId}/{fileId}/{filename}
    if (parts.length >= 5 && parts[0] === 'f' && parts[1] === 'f') {
...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  const signedBlockId =
    ...
    (() => {
      try {
        return new URL(ret).searchParams.get('id')
      } catch {
        return null
      }
    })()

  ret = rewriteFileNotionUrl(ret)
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    ...
    // 签名 URL 追加 t= 会破坏 signature；已转为 /image 代理的地址可以带 t=
    const isSignedFileUrl =
      ret.includes('signature=') || ret.includes('expirationTimestamp=')
    if (
      ret &&
      ret.length > 4 &&
      ...
      ret = `${ret.trim()}${separator}t=${signedBlockId || block?.id || ''}`
    }
  }
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  // Notion 图床 /image 代理（含 attachment: 与 aws 源）
  if (
    image.indexOf(BLOG.NOTION_HOST) === 0 &&
    (image.includes('/image/') || image.includes('amazonaws.com'))
  ) {
