...
import {
  ensureFirstContentImages,
  hydrateFirstContentImages
} from '@/lib/db/notion/getFirstContentImage'
  ...
  try {
    for (let index = 0; index < siteIds.length; index++) {
      ...
      if (index === 0 || locale === prefix) {
        data = await getSiteDataByPageId({ pageId: id, from, skipBatchFetch })
      }
 ...
 */
export async function getSiteDataByPageId({ pageId, from, skipBatchFetch = false }) {
  // const siteStart = Date.now()

  // 轻量模式：跳过批量补拉，不缓存，避免污染缓存
  if (skipBatchFetch) {
    console.log('获取全站数据 (lightweight) ', pageId)
    const originalPageRecordMap = await fetchNotionPageBlocks(pageId, from)
    const r = await convertNotionToSiteData(pageId, from, originalPageRecordMap, { skipBatchFetch: true })
    await ensureFirstContentImages(r?.allPages)
    return r
  }
  ...
  await ensureFirstContentImages(data?.allPages)
...
const EmptyData = pageId => ({
  // 标记失败兜底，禁止写入 Redis/内存缓存，避免修好 API 后仍长时间读到空站
  __notionEmpty: true,
  notice: null,
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  // 拉全站数据 — 优先轻量模式，未命中再回退全量拉取
  const step1Start = Date.now()
  let props = await fetchGlobalAllData({ from: source, locale, skipBatchFetch: true })
  const step1End = Date.now()
  console.log(`[${taskId}] ⏱ fetchGlobalAllData (lightweight) 耗时: ${step1End - step1Start}ms @ ${new Date().toISOString()}`)

  // 工具函数：查找文章
  const findPost = (pages) => {
    if (!pages) return null
    return (
      // 1. 完整 slug 匹配
      pages.find(p => p && !p.type?.includes('Menu') && p.slug === fullSlug) ||
      // 2. UUID 匹配
      pages.find(p => p?.id === fullSlug) ||
      null
    )
  }
  ...
  post = findPost(props.allPages)
  // const step2End = Date.now()

  // 轻量模式未命中 → 回退全量拉取
  if (!post && props.allPages?.length > 0) {
    const fallbackStart = Date.now()
    console.log(`[${taskId}] ⚡ 轻量模式未命中，回退全量拉取`)
    props = await fetchGlobalAllData({ from: source, locale })
    post = findPost(props.allPages)
    console.log(`[${taskId}] ⏱ 全量回退耗时: ${Date.now() - fallbackStart}ms @ ${new Date().toISOString()}`)
  }
...
async function convertNotionToSiteData(
  ...
  const { skipBatchFetch = false } = options
  const traceId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
  const overallStart = Date.now()
  console.log(`[${traceId}] 🕒 开始 convertNotionToSiteData from: ${from} ${skipBatchFetch ? '(lightweight)' : ''} @ ${new Date().toISOString()}`)

  if (!pageRecordMap) {
    console.error(`[${traceId}] can't get Notion Data ; pageId:`, SITE_DATABASE_PAGE_ID)
    // 必须返回完整站点结构；空对象会导致 categoryOptions 等为 undefined，getStaticProps JSON 序列化失败
  ...
  if (!skipBatchFetch && blockIdsNeedFetch.length > 0) {
  ...
  } else if (skipBatchFetch && blockIdsNeedFetch.length > 0) {
    console.log(`[${traceId}] ⚡ 轻量模式：跳过 ${blockIdsNeedFetch.length} 个 block 的批量补拉`)
  }
  ...
  for (const id of pageIds) {
    ...
    if (properties) {
      properties.contentScanIds = (pageBlock.content || []).slice(0, 8)
  ...
  if (!skipBatchFetch) {
    ...
    block = await hydrateFirstContentImages(collectionData, block)
    console.log(`[${traceId}] ⏱ 列表缩略图提取耗时: ${Date.now() - thumbStart}ms @ ${new Date().toISOString()}`)
  }
  ...
  allPages = applyPostListSort(allPages, NOTION_CONFIG)
...
function handleDataBeforeReturn(db) {
  const schemaSnapshot = db.schema
  delete db.block
  delete db.schema
  ...
  // 与标签一致：若数据库「分类」属性里已删除某选项，行上仍可能残留旧值；仅保留当前 schema 中存在的选项
  const validCategories = new Set(
    (getCategoryOptions(schemaSnapshot) || [])
      .map(o =>
  ...
  db.allNavPages = cleanPages(db?.allNavPages, db.tagOptions, validCategories)
  db.allPages = cleanPages(db.allPages, db.tagOptions, validCategories)
  db.latestPosts = cleanPages(db.latestPosts, db.tagOptions, validCategories)
  ...
  // 缓存里可能仍是 Notion 表格顺序；返回前再按配置排序，避免改 POSTS_SORT_BY 后必须清缓存
  db.allPages = applyPostListSort(db.allPages, db.NOTION_CONFIG)

  // latestPosts 在定时发布等逻辑之前生成，且与 allPages 共享引用；此处按最终状态重建，避免首页列表与侧栏数量/条目不一致
  const latestPostCount = siteConfig(
    'LATEST_POST_COUNT',
    BLOG.LATEST_POST_COUNT || 6,
    db.NOTION_CONFIG
  )
  db.latestPosts = getLatestPosts({
    allPages: db.allPages,
    from: 'handleDataBeforeReturn',
...
function cleanPages(allPages, tagOptions, validCategories) {
  ...
  allPages.forEach(page => {
    ...
    if (validCategories instanceof Set) {
...
/**
 * 列表排序：date = 发布时间倒序；notion = 保持 Notion 表格顺序。
 * 开启 TOP_TAG 时，只重排置顶子集。
 */
function applyPostListSort(pages, NOTION_CONFIG) {
  if (!Array.isArray(pages)) return pages
  const sortBy = siteConfig('POSTS_SORT_BY', BLOG.POSTS_SORT_BY, NOTION_CONFIG)
  let next = pages
  if (sortBy === 'date') {
    next = [...pages].sort((a, b) => (b?.publishDate ?? 0) - (a?.publishDate ?? 0))
  }
  const topTag = siteConfig('TOP_TAG', '', NOTION_CONFIG)
  if (topTag) {
    next = sortPinnedPostsByLatestUpdate(next, topTag)
  }
  return next
}
...
function convertToUTC(dateStr, timeZone = 'Asia/Shanghai') {
  ...
  // POSIX Etc/GMT±N is inverted: Etc/GMT-8 == UTC+8, Etc/GMT+8 == UTC-8
  if (offsetHours === undefined && typeof timeZone === 'string') {
    const etc = timeZone.match(/^Etc\/GMT([+-])(\d+(?:\.\d+)?)$/)
    if (etc) {
      offsetHours = (etc[1] === '-' ? 1 : -1) * Number(etc[2])
    }
  }
